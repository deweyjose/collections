...
TEST(SLINKEDLIST, ISEMPTY) {
    slinkedlist<int> slinkedlist;
    ASSERT_TRUE(slinkedlist.isEmpty());
}

TEST(SLINKEDLIST, PUSH_FRONT) {
...
TEST(SLINKEDLIST, POSTPREFIX) {
    slinkedlist<int> list;
    list.push_front(99);
    list.push_front(98);
    list.push_front(97);

    slinkedlist<int>::iterator iter = list.begin();
...
TEST(SLINKEDLIST, ITERATOR) {
    ...
    int i = 4;
    for (slinkedlist<int>::iterator iter = list.begin(); iter != list.end(); ++iter) {
        ASSERT_EQ(i--, *iter);
    }
}

TEST(SLINKEDLIST, CONST_ITERATOR) {
    slinkedlist<test_class> list;
    ...
    int i = 4;
    for (slinkedlist<test_class>::const_iterator citer = list.cbegin(); citer != list.cend(); ++citer) {
        ASSERT_EQ(i--, (*citer).get_test_value());
    }
}

TEST(SLINKEDLIST, SIZE) {
    slinkedlist<test_class> list;
    for (int i = 0; i < 10; ++i) {
        list.push_front(test_class(i));
...
TEST(SLINKEDLIST, ERASE) {
    ...
    for (int i = 0; i < 10; i+=2) {
        slinkedlist<test_class>::iterator iterator = list.begin();
